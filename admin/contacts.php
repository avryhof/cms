<? 
	...
	$contacts = $db->query("SELECT * FROM `contact` ORDER BY `created` DESC");
?>
<!DOCTYPE html>
        ...
        <?
			if ($contacts->num_rows > 0) {
				while($contact = $contacts->fetch_assoc()) {
					$replies = $db->query("SELECT * FROM `replies` WHERE `contact` = $contact[id]");
		?>
        <tr>
        	<td class="text-center"><?= ($replies->num_rows > 0 ? '<i class="fa fa-reply"></i>' : '&nbsp;'); ?></td>
        	<td><?= $contact['name']; ?></td>
            <td><?= $contact['created']; ?></td>
            <td class="text-center"><a href="../admin/contact.php?id=<?= $contact['id']; ?>"><i class="fa fa-pencil"></i></a></td>
        </tr>
        <?
				}
			}
