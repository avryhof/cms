  ...
  <body>
    ...
    <div class="container">
      <div class="row">
        ...
        <a href="page.php?action=add" class="btn btn-primary"><i class="fa fa-plus"></i> Add</a>
        ...
        <tfoot>
        <tr>
        	<td colspan="4">To Rename, Edit, or Delete a page, Click the <i class="fa fa-pencil"></i> icon in the right-hand column.<br>To re-order pages, click and drag the item to a new position with the <i class="fa fa-bars"></i> icon in the sort column.</td>
        </tr>
        </tfoot>
        </table>
			...
			$('.sortable').sortable({
				...
				update: function( event, ui ) {
					$(".sortable").find("tr").each(function(index, element) {
						$.getJSON('page_api.php', {
							action: 'order',
							id: parseInt($(this).attr("id").replace('page_','')),
							order: index
						}, function ( data ) {
							console.log(data);
						});
...
<!-- InstanceEnd --></html>
<?
	function show_pages($parent_id = -1) {
		global $db;
		...
		if ($pages->num_rows > 0) {
			...
			?>
        <tr id="page_<?= $page['id']; ?>">
        	<td class="grabhandle"><i class="fa fa-bars"></i></td>
        	<td class="text-center"><?= ($page['active'] == 1 ? '<i class="fa fa-check-square-o"></i>' : '<i class="fa fa-square-o"></i>'); ?></td>
        	<td><?= ($page['parent'] > 0 ? '<i class="fa fa-arrow-left"></i> ' : ''); ?><?= $page['title']; ?></td>
            <td class="text-center"><a href="../admin/page.php?id=<?= $page['id']; ?>"><i class="fa fa-pencil"></i></a></td>
        </tr>
            <?
				show_pages($page['id']);
			}
