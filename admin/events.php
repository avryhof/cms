<? 
	...
	$events = $db->query("SELECT * FROM `events` ORDER BY `begin`,`complete`");
?>
<!DOCTYPE html>
  ...
  <body>
    ...
    <div class="container">
      <div class="row">
        ...
        <div class="btn-group">
            <a href="../<?= $admin_folder; ?>/event.php?action=add" class="btn btn-primary"><i class="fa fa-plus"></i> Add</a>
            <a href="../<?= $admin_folder; ?>/events_import.php" class="btn btn-success"><i class="fa fa-upload"></i> Import</a>
        </div>
		...
		?>
        <tr>
        	<td class="text-center"><?= ($event['active'] == 1 ? '<i class="fa fa-check-square-o"></i>' : '<i class="fa fa-square-o"></i>'); ?></td>
        	<td><?= date("Y-m-d g:i a",strtotime($event['begin'])); ?></td>
            <td><?= ($event['complete'] !== '0000-00-00 00:00:00' ? date("Y-m-d g:i a",strtotime($event['complete'])) : ''); ?></td>
            <td><?= $event['title']; ?></td>
            <td class="text-center"><a href="../<?= $admin_folder; ?>/event.php?id=<?= $event['id']; ?>"><i class="fa fa-pencil"></i></a></td>
        </tr>
        <?
				}
			}
